...
async function espDisconnect(reselect = false) {
	console.log("log: disconnection start");
	if (!espPort)
		return;
...
async function espFlash(binData = [], offset) {
	console.log("log: espflash start");
	for (let i = 0; i < binData.length; i++) {
		...
		for (let seq of Array(blocks).keys()) {
			...
			console.log("log: Flash Write:" + (seq + 1) + "/" + blocks + "\n");
...
function addSerial(msg) {
	...
	if($('#outputSrea').hasClass('is-auto-scroll')) {
		console.log("has class 'is-auto-scroll'");
		textarea.scrollTop = textarea.scrollHeight;
	}*/
}
